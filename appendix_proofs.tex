...
% \old{Lemma~\ref{lemma:proximity_to_solution_small_loss} below shows that if the parameters are within close proximity to an extrapolating solution, the generalization error (i.e. loss values) are small.
...
% \begin{lemma}\label{lemma:normalized_student_approximator}
%     Consider an RNN with bounded parameters $\Theta$, satisfying $(\mC\mB-1)^2\le\delta$.
%     Then $\forall j=\mathbb{N}$,
%     \begin{equation}
%         \left( \frac{\mC\mA^j\mB}{\mC\mB}-\mC\mA^j\mB \right)^2=\Oc (\delta).
%     \end{equation}
% \end{lemma}

% \begin{proof}[Proof of Lemma~\ref{lemma:normalized_student_approximator}]

% Denote $\beta\equiv\mC\mB-1$, from our assumptions, $\beta\in \left[ -\sqrt{\delta}, \sqrt{\delta}\right]$, the gap between the normalized student and the original impulse responses satisfies, $\forall j\in\mathbb{N}$

% \begin{align}
%     \left(\frac{\mC\mA^j\mB}{1+\beta}- \mC\mA^j\mB\right)^2 & = (1+\beta)^{-2}\left(\mC\mA^j\mB- (1+\beta)\mC\mA^j\mB\right)^2\\
%     & = (1+\beta)^{-2}\left( \beta\mC\mA^j\mB \right)^2
% \end{align}
% For a reasonably small $\delta$ (e.g. $\delta<0.1$), the above can be bounded by 
% \begin{equation}
%     \left(\frac{\mC\mA^j\mB}{1+\beta}- \mC\mA^j\mB\right)^2\leq 2\delta\left(\mC\mA^j\mB\right)^2 \leq 2\delta\left(\|\mC\|_2\|\mA^j\|_2\|\mB\|_2\right)^2\le 2\delta R^{2j+2} =\Oc\left(\delta\right)
% \end{equation}
% where the second transition follows from Couchy-Swartz inequality and the third transition uses the upper bound ($R$) on the parameters.
% \end{proof}
% }
...
% \old{Assume the conditions of Theorem~\ref{thm:exact_extrapolation}, and that the teacher parameters~$\hat{\Theta}$ are stable, i.e. the eigenvalues of~\smash{$\hat{\mA}$} are in~$[-1,1]$.
...
\begin{align}
    ...
    & = \left( \mC\mA^j\mB\left(\frac{1}{\hat{\mC}\hat{\mB}} - \frac{1}{\mC\mB}\right) + \underbrace{\frac{\mC\mA^j\mB}{\mC\mB} -\frac{\hat{\mC}\hat{\mA}^j\hat{\mB}}{\hat{\mC}\hat{\mB}}}_{m_j-\hat{m}_j}\right)^2 %\leq \frac{\delta}{(\hat{\mC}\hat{\mB})^{2}}. \\
     % & = \sum_{j=0}^{k-1} \left( \mC\mA^j\mB -\hat{\mC}\hat{\mA}^j\hat{\mB}\right)^2
\end{align}
...
% From \Eqref{eq:loss_smaller_than_delta}, we have $(\mC\mB-\hat{\mC}\hat{\mB})^2\leq \delta$.
% % specifically, $|\mC\mB-\hat{\mC}\hat{\mB}|\leq\sqrt{\delta}$. 


% In order to shift the discussion to that of moment matching, we show that the `normalized' student (for which $\mC\mB=1$) is a good approximator to the original student. The above is described by  

% Next, we show that 


% We shall continue the analysis under the assumption that $m_0=1$ and justify this later by showing the introduced error by the said approximation is small. From \Eqref{eq:loss_smaller_than_delta} we have $(m_j - \hat{m}_j)^2\le \delta$ for $j=1,\dots,k-1$ (and $m_0=\hat{m}_0=1$).
% From assumption (i) $k>2\hat{d}$ and specifically $k-1\ge 2\hat{d}$, therefore any $\Theta$ satisfying $\mathcal{L}(\Theta)\le \delta$ satisfies in particular $|m_j-\hat{m}_j|\le \delta^{0.5}$ for $j=1,\dots,2\hat{d}$.
...
% where $\hat{m}_0$ and $m_0$ are defined to be $\hat{\mC}\hat{\mB}$ and $\mC\mB$ respectively (recall $\hat{m}_0=1$ per our assumption). From \Eqref{eq:loss_smaller_than_delta} we have 
% that for every $j=0,\dots,k-1$, $(m_j-\hat{m}_j)^2\le \delta$.
% We shall continue with the normalized student for which $m_0=1$ and justify this assumption later on.




% Additionally, each term above satisfies $|\mC\mA^j\mB -\hat{m}_j |\leq \sqrt{\delta}$ for $j=1,\dots,k-1$. For $j=0$, $|\mC\mB-1|\leq \sqrt{\delta}$.  

% The extrapolation error of the normalized teacher and student amounts to the difference in their $j^{th}$ moment for $j\ge k$
% In order to show $\epsilon$-extrapolation with horizon~$q$ we need to find $\delta(\epsilon,q)$ such that if $\mathcal{L}(\Theta)\le \delta(\epsilon,q)$
% After establishing that the normalized student is 
% % The above establishes that considering the normalized student 
...
% We establish \Eqref{eq:moments_approx} by employing the theory of Wasserstein distances \nadav{cite} \ec{we should probably define Wasserstein distance in \Secref{sec:setup}}.
